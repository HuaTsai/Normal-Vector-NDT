...
pcl::PointCloud<pcl::PointXYZL>::Ptr segmented_not_ground_pc(
    new pcl::PointCloud<pcl::PointXYZL>());
pcl::PointCloud<pcl::PointXYZRGB>::Ptr g_color_pc(
...
class GroundPlaneFit {
 ...
 private:
  ...
  void publishCloud(const ros::Publisher*,
                    const pcl::PointCloud<pcl::PointXYZI>::Ptr);
  void publishCloudRGB(const ros::Publisher*,
...
GroundPlaneFit::GroundPlaneFit() : node_handle_("~") {
  ...
  points_node_sub_ = node_handle_.subscribe(
      point_topic_, 300, &GroundPlaneFit::velodyne_callback_, this);
  ...
  groundless_points_pub_ = node_handle_.advertise<sensor_msgs::PointCloud2>(
      no_ground_topic, 100, true);
  ground_points_pub_ =
      node_handle_.advertise<sensor_msgs::PointCloud2>(ground_topic, 300);
  color_points_pub_ = node_handle_.advertise<sensor_msgs::PointCloud2>(
      "/plane_color_points", 100, true);
  // all_points_pub_ =
  // node_handle_.advertise<sensor_msgs::PointCloud2>("/all_points", 100);
  // lowest_point_pub_ =
  // node_handle_.advertise<sensor_msgs::PointCloud2>("/lowest_points", 100,
  // true); seeds_pub_ =
  // node_handle_.advertise<sensor_msgs::PointCloud2>("/seeds_points", 100,
  // true); ego_points_pub_ =
  // node_handle_.advertise<sensor_msgs::PointCloud2>("/ego_points", 100, true);
  // accuracy_pub_ =
  // node_handle_.advertise<geometry_msgs::PointStamped>("/plane_accuracy",
  // 100); marker_pub_ =
  // node_handle_.advertise<visualization_msgs::MarkerArray>("/marker_array",
  // 100, true);
  ...
  for (int r = 0; r < n; ++r) {
    ...
    for (int c = 0; c < m; ++c) {
      for (int k = 0; k <= angular_resolution; k++) {
        ...
        c_p.x = (r + 1) * grid_dist *
                cos(angle * (c) + angle * (k) / angular_resolution);
        c_p.y = (r + 1) * grid_dist *
                sin(angle * (c) + angle * (k) / angular_resolution);
        c_p.z = -1.9;
      }
    }
...
void GroundPlaneFit::extract_initial_seeds(
  ...
  for (size_t i = 0; i < p_sorted.points.size() && cnt < num_lpr_; i++) {
    if (p_sorted.points[i].z > -3.5) {
      ...
      // cout<<"index: "<<p_sorted.points[i].label<<" z:
      // "<<p_sorted.points[i].z<<endl;
    }
  }
...
void GroundPlaneFit::horizontal_seg(
  ...
  for (size_t i = 0; i < laserCloudIn.points.size(); i++) {
    ...
    if (hor_seg > m - 1 || hor_seg < 0) {
      ...
      // cout<<"m: "<<m<<" n: "<<n<<" hor_seg: "<<hor_seg<<" vert_seg:
      // "<<vert_seg<<endl;
    ...
    // cout<<"push_back: "<<i<<" data["<<hor_seg<<"]"<<"["<<vert_seg<<"]:
    // "<<data[hor_seg][vert_seg].points().size()<<endl;
  }
}
...
void GroundPlaneFit::extract_ground(
  ...
  for (int i = 0; i < num_iter_; i++) {
    // cout<<"iter: "<<i<<" points_size:
    // "<<segmented_ground_pc->points.size()<<endl;
    estimate_plane(i);
    segmented_ground_pc->clear();
    segmented_not_ground_pc->clear();
...
void GroundPlaneFit::publishCloud(
...
void GroundPlaneFit::publishCloudRGB(
    const ros::Publisher* in_publisher,
...
void GroundPlaneFit::velodyne_callback_(
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr ground_seg(
      new pcl::PointCloud<pcl::PointXYZI>());
  pcl::PointCloud<pcl::PointXYZI>::Ptr nonground_seg(
      new pcl::PointCloud<pcl::PointXYZI>());
  ...
  // cout<<"true_points: "<<true_points<<" size:
  // "<<g_all_pc->points.size()<<endl; cout<<"Sequence: "<<counter<<" Accuracy:
  // "<<accuracy<<" Total_accuracy: "<<total_accuracy/counter<<endl;
