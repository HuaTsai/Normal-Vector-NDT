...
class Normal2dEstimation {
  ...
  void compute(const PtrCloud& normal_cloud) const {
    ...
    if (m_in_cloud->is_dense) {
      ...
      for (unsigned int idx = 0; idx < m_indices->size(); ++idx) {
        if (this->searchForNeighbors((*m_indices)[idx], *nn_indices,
                                     nn_dists) == 0) {
      ...
      for (unsigned int idx = 0; idx < m_indices->size(); ++idx) {
        if (!isFinite(m_in_cloud->points[(*m_indices)[idx]]) ||
            this->searchForNeighbors((*m_indices)[idx], *nn_indices,
                                     nn_dists) == 0) {
          normal_cloud->points[idx].x = normal_cloud->points[idx].y =
              normal_cloud->points[idx].z =
                  std::numeric_limits<float>::quiet_NaN();
 ...
 private:
  int searchForNeighbors(int index, std::vector<int>& nn_indices,
    ...
    nn_dists.clear();
    if (m_k == 0) {
      this->m_kd_tree->radiusSearch(index, m_search_radius, nn_indices,
                                    nn_dists, 0);
    } else {
  ...
  void flipNormalTowardsViewpoint(const Point& point,
                                  Eigen::Matrix<double, 3, 1>& normal) const {
    Eigen::Matrix<double, 3, 1> vp(m_view_point.x - point.x,
                                   m_view_point.y - point.y, 0.);
  ...
  void flipNormalTowardsViewpoint(const Point& point, float& x, float& y,
                                  float& z) const {
