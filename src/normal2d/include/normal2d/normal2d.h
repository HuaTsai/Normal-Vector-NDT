...
/** \brief Normal2dEstimation estimate local straight line at each points using
 * it neighbour. Contrary to native normal estimation  on PCL lib, it works only
 * on 2D points cloud.
 *
 * \note This class use 3D points cloud but ignore all z components. So make
 * sure you have all your points on z=0 plan.
 *
 *
 */
class Normal2dEstimation {
 public:
  /**
   * \brief Classic constructor, you can directly setup parameters using
   * constructor arguments.
   *
   * @param search_radius If different from zero, the normal estimation will use
   * the radius to find out which points it take for line estimation.
   *
   * @param k If different from zero, the normal estimation will find the kth
   * closest points for computing line estimation.
   */
   ...
   *
   *  The view points is used in normal orientation purpose. This orientation
   * force all normals to look at view points.
   *
  ...
  /**
   * \brief Provide a pointer to the vector of indices that represents the input
   * data. \param indices indices a pointer to the indices that represent the
   * input data.
   */
 ...
 private:
  /** \brief Search for nearest neighbors using the spatial locator from search
   * class and class parameters (k and search_radius).
   *
   * \param index index the index of the query point.
   * \param[out] nn_indices the resultant vector of indices representing the
   * neighbors cluster. \param[out] nn_dists the resultant vector of distances
   * representing the distances from the query point to the neighbors.
   *
  ...
  /** \brief Compute the Least-Squares straight line fit for a given set of
    points, using their indices,
  ...
  /** \brief Compute the Least-Squares straight line fit for a given set of
   * points, using their indices, and return the estimated normal parameters.
   *
   * \param indices the point cloud indices that need to be used.
   * \param nx the resultant X component of the plane normal.
   * \param ny the resultant Y component of the plane normal.
  ...
  double m_search_radius;  ///< Max distance between the estimate normal
                           ///< position with each neighbour for take them on
                           ///< normal estimation.
