...
/***
 *  \brief A self implemented PCA using only 2 dim. That ensure to always have a
 * normal estimation on z=0 plan.
 */
class PCA2D {
 public:
   ...
   */
  PCA2D()
      : m_computed(false), m_indices(new std::vector<int>), m_cloud(nullptr) {}
  ...
  /**
   *  \brief Provide an input cloud pointer to deal with.
  ...
  /** \brief Return the mean computed, if PCA isn't computed, it will perform
   * it.
   *
   * @return The mean vector from PCA algorithm.
   */
  Eigen::Vector2f getMean() {
    if (!m_computed) initCompute();
    return m_mean;
  }

  /** \brief Return the eigen Vector in Eigen Matrix , if PCA isn't computed, it
   * will perform it.
   *
