...
Eigen::Affine2d ICPMatch(const std::vector<Eigen::Vector2d> &tpts,
  ...
  while (params._converge == Converge::kNotConverge) {
    ...
    // orj.RangeRejection(ps, qs, Rejection::kThreshold, 2.5);
...
Eigen::Affine2d SICPMatch(const std::vector<Eigen::Vector2d> &target_points,
  ...
  while (params._converge == Converge::kNotConverge) {
    ...
    OutlierRejectionMaker orj(ps.size());
    // orj.RangeRejection(ps, qs, Rejection::kThreshold, 2.5);
    // orj.AngleRejection(nps, nqs, Rejection::kThreshold, 0.2);
    auto ids = orj.indices();
    for (auto id : ids)
    ...
    // opt.AddResidualBlock(SICPCostFunctor2::Create(ps, nps, qs, nqs));
    params._usedtime.ProcedureFinish();
...
Eigen::Affine2d D2DNDTMatch(const NDTMap &target_map,
  ...
  while (params._converge == Converge::kNotConverge) {
    ...
    // orj.RangeRejection(ups, uqs, Rejection::kThreshold, {2});
    orj.RangeRejection(ups, uqs, Rejection::kStatistic, {1.5});
    // orj.AngleRejection(nps, nqs, Rejection::kThreshold, {0.1});
    // orj.AngleRejection(nps, nqs, Rejection::kStatistic, {1});
    auto ids = orj.indices();
...
Eigen::Affine2d SNDTMatch2(const NDTMap &target_map,
  ...
  while (params._converge == Converge::kNotConverge) {
    ...
    OutlierRejectionMaker orj(ups.size());
    // orj.RangeRejection(ups, uqs, Rejection::kThreshold, {0.1});
    orj.RangeRejection(ups, uqs, Rejection::kStatistic, {1.5});
    // orj.AngleRejection(nps, nqs, Rejection::kBoth, {0, 0.1});
    // if (params._iteration == 0)
    // for (size_t i = 0; i < nps.size(); ++i)
    // printf("%.2f, ", std::acos(nps[i].dot(nqs[i])));
    // std::cout << std::endl;

    // orj.AngleRejection(nps, nqs, Rejection::kStatistic, {1});
    auto ids = orj.indices();
    // printf("sndt: %ld, ", ids.size());
    RetainIndices(ups, ids);
    RetainIndices(uqs, ids);
    RetainIndices(cps, ids);
    RetainIndices(cqs, ids);
