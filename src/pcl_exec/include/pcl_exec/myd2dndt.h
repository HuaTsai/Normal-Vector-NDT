...
protected:
  ...
  void
  computeDerivativeComponents(const Eigen::Matrix<double, 6, 1>& transform,
                              bool compute_hessian = true);
  ...
  Eigen::Matrix<double, 8, 3> jacom_;
  Eigen::Matrix<double, 15, 3> Habcom_;
  Eigen::Matrix<double, 9, 3> Zacom_;
  Eigen::Matrix<double, 18, 3> Zabcom_;
    ...
    bool compute_hessian)
{
  ...
  hessian.setZero();
  double score = 0;
  computeDerivativeComponents(transform);
...
template <typename PointSource, typename PointTarget>
void
NormalDistributionsTransformD2D<PointSource, PointTarget>::computeDerivativeComponents(
    const Eigen::Matrix<double, 6, 1>& transform, bool compute_hessian)
{
  ...
  jacom_.row(0) << -sx * sz + sy * cx * cz, -sx * cz - sy * sz * cx, -cx * cy;
  jacom_.row(1) << sx * sy * cz + sz * cx, -sx * sy * sz + cx * cz, -sx * cy;
  jacom_.row(2) << -sy * cz, sy * sz, cy;
  jacom_.row(3) << sx * cy * cz, -sx * sz * cy, sx * sy;
  jacom_.row(4) << -cx * cy * cz, sz * cx * cy, -sy * cx;
  jacom_.row(5) << -sz * cy, -cy * cz, 0.;
  jacom_.row(6) << -sx * sy * sz + cx * cz, -sx * sy * cz - sz * cx, 0.;
  jacom_.row(7) << sx * cz + sy * sz * cx, -sx * sz + sy * cx * cz, 0.;

  Zacom_.row(0).setZero();
  Zacom_.row(1) << -sx * sz + sy * cx * cz, -sx * cz - sy * sz * cx, -cx * cy;
  Zacom_.row(2) << sx * sy * cz + sz * cx, -sx * sy * sz + cx * cz, -sx * cy;
  Zacom_.row(3) << -sy * cz, sy * sz, cy;
  Zacom_.row(4) << sx * cy * cz, -sx * sz * cy, sx * sy;
  Zacom_.row(5) << -cx * cy * cz, sz * cx * cy, -sy * cx;
  Zacom_.row(6) << -sz * cy, -cy * cz, 0.;
  Zacom_.row(7) << -sx * sy * sz + cx * cz, -sx * sy * cz - sz * cx, 0.;
  Zacom_.row(8) << sx * cz + sy * sz * cx, -sx * sz + sy * cx * cz, 0.;

  if (compute_hessian) {
    Habcom_.row(0) << -sx * sy * cz - sz * cx, sx * sy * sz - cx * cz, sx * cy;
    Habcom_.row(1) << -sx * sz + sy * cx * cz, -sx * cz - sy * sz * cx, -cx * cy;
    Habcom_.row(2) << cx * cy * cz, -sz * cx * cy, sy * cx;
    Habcom_.row(3) << sx * cy * cz, -sx * sz * cy, sx * sy;
    Habcom_.row(4) << -sx * cz - sy * sz * cx, sx * sz - sy * cx * cz, 0.;
    Habcom_.row(5) << -sx * sy * sz + cx * cz, -sx * sy * cz - sz * cx, 0.;
    Habcom_.row(6) << -cy * cz, sz * cy, -sy;
    Habcom_.row(7) << -sx * sy * cz, sx * sy * sz, sx * cy;
    Habcom_.row(8) << sy * cx * cz, -sy * sz * cx, -cx * cy;
    Habcom_.row(9) << sy * sz, sy * cz, 0.;
    Habcom_.row(10) << -sx * sz * cy, -sx * cy * cz, 0.;
    Habcom_.row(11) << sz * cx * cy, cx * cy * cz, 0.;
    Habcom_.row(12) << -cy * cz, sz * cy, 0.;
    Habcom_.row(13) << -sx * sy * cz - sz * cx, sx * sy * sz - cx * cz, 0.;
    Habcom_.row(14) << -sx * sz + sy * cx * cz, -sx * cz - sy * sz * cx, 0.;

    Zabcom_.row(0).setZero();
    Zabcom_.row(1) << -sx * sy * cz - sz * cx, sx * sy * sz - cx * cz, sx * cy;
    Zabcom_.row(2) << -sx * sz + sy * cx * cz, -sx * cz - sy * sz * cx, -cx * cy;
    Zabcom_.row(3).setZero();
    Zabcom_.row(4) << cx * cy * cz, -sz * cx * cy, sy * cx;
    Zabcom_.row(5) << sx * cy * cz, -sx * sz * cy, sx * sy;
    Zabcom_.row(6).setZero();
    Zabcom_.row(7) << -sx * cz - sy * sz * cx, sx * sz - sy * cx * cz, 0.;
    Zabcom_.row(8) << -sx * sy * sz + cx * cz, -sx * sy * cz - sz * cx, 0.;
    Zabcom_.row(9) << -cy * cz, sz * cy, -sy;
    Zabcom_.row(10) << -sx * sy * cz, sx * sy * sz, sx * cy;
    Zabcom_.row(11) << sy * cx * cz, -sy * sz * cx, -cx * cy;
    Zabcom_.row(12) << sy * sz, sy * cz, 0.;
    Zabcom_.row(13) << -sx * sz * cy, -sx * cy * cz, 0.;
    Zabcom_.row(14) << sz * cx * cy, cx * cy * cz, 0.;
    Zabcom_.row(15) << -cy * cz, sz * cy, 0.;
    Zabcom_.row(16) << -sx * sy * cz - sz * cx, sx * sy * sz - cx * cz, 0.;
    Zabcom_.row(17) << -sx * sz + sy * cx * cz, -sx * cz - sy * sz * cx, 0.;
  }
  // clang-format on
}
...
{
  Eigen::Matrix<double, 8, 1> j = jacom_ * mean;
  jas_.col(0) = Eigen::Vector3d::UnitX();
  jas_.col(1) = Eigen::Vector3d::UnitY();
  jas_.col(2) = Eigen::Vector3d::UnitZ();
  jas_.col(3) << 0, j(0), j(1);
  jas_.col(4) << j(2), j(3), j(4);
  jas_.col(5) << j(5), j(6), j(7);

  Eigen::Matrix<double, 9, 3> za = Zacom_ * cov * R_.transpose();
  Zas_.setZero();
  ...
  if (compute_hessian) {
    Eigen::Matrix<double, 15, 1> h = Habcom_ * mean;
    Habs_.setZero();
    Habs_.block<3, 1>(9, 3) << 0, h(0), h(1);
    Habs_.block<3, 1>(9, 4) << 0, h(2), h(3);
    Habs_.block<3, 1>(9, 5) << 0, h(4), h(5);
    Habs_.block<3, 1>(12, 3) = Habs_.block<3, 1>(9, 4);
    Habs_.block<3, 1>(12, 4) << h(6), h(7), h(8);
    Habs_.block<3, 1>(12, 5) << h(9), h(10), h(11);
    Habs_.block<3, 1>(15, 3) = Habs_.block<3, 1>(9, 5);
    Habs_.block<3, 1>(15, 4) = Habs_.block<3, 1>(12, 5);
    Habs_.block<3, 1>(15, 5) << h(12), h(13), h(14);

    Eigen::Matrix<double, 18, 3> z = Zabcom_ * cov * R_.transpose();
    int i = 0;
    for (int a = 0; a < 3; ++a) {
      for (int b = a; b < 3; ++b) {
        z.block<3, 3>(i * 3, 0) +=
