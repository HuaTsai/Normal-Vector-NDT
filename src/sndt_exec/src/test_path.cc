...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < n - f; i += f) {
    ...
    vector<Vector2d> src3 =
        AugmentPoints(vepcs, i + f, i + 2 * f - 1, Toq, Toqs);
    auto srctime = vepcs[i + f].stamp;
    if (bagout) bag.write("source", srctime, EigenToPC(src3, srctime));
    ...
    if (bagout) bag.write("sndt", srctime, EigenToPC(next1, srctime));
    auto t2 = GetTime();
    ...
    if (bagout) bag.write("ndt", srctime, EigenToPC(next2, srctime));
    auto t4 = GetTime();
    ...
    if (bagout) bag.write("sicp", srctime, EigenToPC(next3, srctime));
    auto t6 = GetTime();
