...
void cb(const std_msgs::Int32 &num) {
  ...
  double cost2 = SNDTCostFunctor2::Cost(up, cp, unp, cnp, uq, cq, unq, cnq, aff);
  ...
  // printf("unp = np.array([[%f], [%f]])\n", unp(0), unp(1));
  // printf("cnp = np.array([[%f, %f], [%f, %f]])\n", cnp(0, 0), cnp(0, 1), cnp(1, 0), cnp(1, 1));
  // printf("uq = np.array([[%f], [%f]])\n", uq(0), uq(1));
  // printf("cq = np.array([[%f, %f], [%f, %f]])\n", cq(0, 0), cq(0, 1), cq(1, 0), cq(1, 1));
  // printf("unq = np.array([[%f], [%f]])\n", unq(0), unq(1));
  // printf("cnq = np.array([[%f, %f], [%f, %f]])\n", cnq(0, 0), cnq(0, 1), cnq(1, 0), cnq(1, 1));
  // cout << "SNDT      => " << cost << ", " << 0.5 * cost * cost << endl;
  // cout << "SNDT(gt)  => " << cost2 << ", " << 0.5 * cost2 * cost2 << endl;
  // cout << "SNDT2     => " << cost3 << ", " << 0.5 * cost3 * cost3 << endl;
  // cout << "SNDT2(gt) => " << cost4 << ", " << 0.5 * cost4 * cost4 << endl;
  // cout << "D2D       => " << cost5 << ", " << 0.5 * cost5 * cost5 << endl;
  // cout << "D2D(gt)   => " << cost6 << ", " << 0.5 * cost6 * cost6 << endl;
}
...
int main(int argc, char **argv) {
  ...
  for (auto aff : affs) {
    ...
    // for (size_t i = 0; i < corr.size(); ++i) {
      ...
      auto Tx = SNDTCellMatch(tcell, scell, params, Eigen::Affine2d::Identity(), m);
      ...
      // cout << "x = ["; for (auto x : xs) cout << x << ", "; cout << "]" << endl;
      // cout << "y = ["; for (auto y : ys) cout << y << ", "; cout << "]" << endl;
      // cout << "t = ["; for (auto t : ts) cout << t << ", "; cout << "]" << endl;
      ...
      us.push_back({tcell->GetPointMean(), scell->GetPointMean(), c->GetPointMean()});
      uns.push_back({tcell->GetNormalMean(), scell->GetNormalMean(), c->GetNormalMean()});
      cs.push_back({tcell->GetPointCov(), scell->GetPointCov(), c->GetPointCov()});
      cns.push_back({tcell->GetNormalCov(), scell->GetNormalCov(), c->GetNormalCov()});
      std_msgs::Int32 num;
      num.data = 0; cb(num);
      ros::spin();
    // }
